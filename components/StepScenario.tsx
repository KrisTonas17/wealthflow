...
export default function StepScenario({ incomeData, budgetData, allocationData, monthlyInvesting, onNext, onBack }: Props) {
  const base = useMemo(() => {
    const b = getBase(incomeData, budgetData, allocationData)
    // Override totalDeployed with the actual portfolio number from the invest step
    return { ...b, totalDeployed: monthlyInvesting }
  }, [incomeData, budgetData, allocationData, monthlyInvesting])
