...
function getMonthlyInvesting(incomeData: IncomeData, allocationData: AllocationData): number {
  ...
  return monthlyNet * (allocationData.investing / 100)
}
...
export default function StepInvestments({ incomeData, allocationData, onNext, onBack }: Props) {
  const suggestedMonthly = getMonthlyInvesting(incomeData, allocationData)

  const [accounts, setAccounts] = useState<InvestmentAccount[]>(
    ACCOUNT_TEMPLATES.map((t, i) => ({
      ...t,
      id: `acc-${i}`,
      monthlyContribution: i === 0 ? Math.round(suggestedMonthly) : t.monthlyContribution,
    }))
  )
  ...
  return (
    <div className="fade-up">
      ...
      <div style={{
        ...
        padding: '12px 18px', marginBottom: 20, display: 'flex', gap: 12, alignItems: 'center',
      }}>
        <span style={{ fontSize: 20 }}>💡</span>
        <span style={{ fontSize: 13, color: 'var(--text-muted)' }}>
          Based on your {allocationData.investing}% investing allocation, your suggested monthly contribution is{' '}
          <strong style={{ color: 'var(--green)' }}>{formatCurrency(suggestedMonthly)}/mo</strong>
        </span>
      </div>
              ...
              <div style={{
                ...
                {acc.type === 'home' && acc.homeValue && acc.loanAmount && acc.mortgageRate && acc.loanTermYears && (
                  <div style={{ marginTop: 12, padding: '10px 14px', background: 'var(--bg)', borderRadius: 8, border: '1px solid var(--border)', display: 'flex', gap: 20, flexWrap: 'wrap' }}>
                    <div>
                      <div style={{ fontSize: 10, color: 'var(--text-muted)' }}>Monthly P&I</div>
                    ...
                    <div>
                      ...
                      <div className="mono" style={{ fontSize: 14, color: acc.color }}>
                        {formatCurrency(acc.homeValue - acc.loanAmount)}
                      </div>
                    </div>
                  </div>
                )}
