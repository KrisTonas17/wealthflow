...
interface Props {
  ...
  onAccountsChange: (accounts: InvestmentAccount[]) => void
  horizon: number
...
export const ACCOUNT_TEMPLATES: Omit<InvestmentAccount, 'id'>[] = [
  {
    type: 'investment',
...
export default function StepInvestments({ incomeData, budgetData, allocationData, accounts, onAccountsChange, horizon, onHorizonChange, onNext, onBack }: Props) {
  ...
  const setHorizon = onHorizonChange
  ...
  // Remaining to allocate: how much of the suggested total isn't yet assigned across enabled accounts
  const allocatedToAccounts = accounts.filter(a => a.enabled).reduce((sum, a) => sum + a.monthlyContribution, 0)
  const remaining = suggestedTotal - allocatedToAccounts
  const remainingPct = suggestedTotal > 0 ? Math.min(100, Math.max(0, (allocatedToAccounts / suggestedTotal) * 100)) : 0
  const isOver = allocatedToAccounts > suggestedTotal
  ...
  return (
    <div className="fade-up">
      ...
      <div style={{
        ...
        border: `1px solid ${isOver ? 'rgba(255,87,87,0.3)' : remaining <= 0 ? 'rgba(34,209,122,0.3)' : 'var(--border)'}`,
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
          ...
          <span className="mono" style={{
            ...
            color: isOver ? 'var(--red)' : remaining <= 1 ? 'var(--green)' : 'var(--text-muted)'
          }}>
            {isOver
        ...
        {/* Progress bar */}
        <div style={{ height: 6, background: 'var(--border-bright)', borderRadius: 3, overflow: 'hidden', marginBottom: 8 }}>
          <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: 'var(--text-faint)' }}>
          <span>{formatCurrency(allocatedToAccounts)}/mo assigned across {accounts.filter(a => a.enabled).length} asset{accounts.filter(a => a.enabled).length !== 1 ? 's' : ''}</span>
          <span>Budget: {formatCurrency(suggestedTotal)}/mo</span>
        </div>
      </div>
