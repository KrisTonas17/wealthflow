  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          {STEPS.map((s) => (
            <button key={s.id} onClick={() => s.id <= step ? setStep(s.id) : undefined}
              ...
              <span>{s.label}</span>
              {s.id < step && <span style={{ color: 'var(--green)', fontSize: 9 }}>✓</span>}
            </button>
          ))}
        </div>
      ...
      <main style={{ maxWidth: 960, margin: '0 auto', padding: '32px 20px 80px' }}>
        ...
        {step === 4 && <StepGoal incomeData={incomeData} budgetData={budgetData} allocationData={allocationData} onNext={() => setStep(5)} onBack={() => setStep(3)} onRestart={() => setStep(0)} />}
        {step === 5 && <StepLifestyle incomeData={incomeData} budgetData={budgetData} allocationData={allocationData} onBack={() => setStep(4)} onRestart={() => setStep(0)} />}
      </main>
    </div>
  )
}
